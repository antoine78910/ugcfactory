...
/**
 * Free-plan users must bring their own Kie (or PiAPI) key so usage is billed on
 * their provider account, not the platform key. Paid plans may use platform credits.
 */
export async function assertPersonalApiForFreePlan(opts: {
  ...
  const hasKey =
    hasPersonalApiKey(opts.personalApiKey) || hasPersonalApiKey(opts.piapiApiKey);
  ...
  return NextResponse.json(
    {
      ...
      message:
        "Add your Kie API key to generate on the free plan. Usage is billed on your Kie account, not platform credits.",
    },
    { status: 403 },
  );
}
