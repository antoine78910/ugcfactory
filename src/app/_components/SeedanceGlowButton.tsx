...
function Letter({ children }: { children: string }) {
  return <span className={styles.btnLetter}>{children}</span>;
}
...
export function SeedanceGlowButton({
  ...
  const focused = (label === "Generate" ? "Generating" : label).split("");
  ...
  return (
    <div className={styles.wrapper}>
      <Link className={`${styles.btn} ${className ?? ""}`} href={href} prefetch={false}>
        ...
        <span className={styles.txtWrapper} aria-label={label}>
          <span className={styles.txt1} aria-hidden>
            {primary.map((c, idx) => (
              <Letter key={`${c}-${idx}`}>{c}</Letter>
            ))}
          </span>
          <span className={styles.txt2} aria-hidden>
            {focused.map((c, idx) => (
              <Letter key={`${c}-${idx}`}>{c}</Letter>
            ))}
          </span>
